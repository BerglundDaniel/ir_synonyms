...
public class SearchGUI extends JFrame {
    ...
    public static final String homeDir = "C:/Users/wilhelm/Skola/Infosok";
	...
	Action search = new AbstractAction() {
		public void actionPerformed( ActionEvent e ) {
			...
			for ( int i=0; i<results.size(); i++ ) {
				buf.append( " " + i + ". " );
				String filename = indexer.index.docIDs.get( "" + results.getPosting(i).docID );
				if ( filename == null ) {
				buf.append( "" + results.getPosting(i).docID );
				}
				else {
				buf.append( filename );
				}
				if ( queryType == Index.RANKED_QUERY ) {
				buf.append( "   " + String.format( "%.5f", results.getPosting(i).score )); 
				}
				buf.append( "\n" );
			}
		    }
